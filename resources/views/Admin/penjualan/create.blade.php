    ...
    <div class="container">
        ...
        <form action="{{ route('admin.penjualan.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" id="cari_kode_barang" class="form-control"
                    placeholder="Masukkan kode barang lalu tekan Enter">
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateSubtotal(row) {
                const harga = parseFloat(row.querySelector('.barang-select').selectedOptions[0]?.getAttribute(
                    'data-harga') || 0);
            ...
            document.getElementById('cari_kode_barang').addEventListener('keydown', function(e) {
                if (e.key === 'Enter') {
                    ...
                    const barangOption = document.querySelector(
                        `.barang-select option[data-kode="${kode}"]`);
                    if (barangOption) {
                        const container = document.getElementById('barang-container');

                        let targetRow = Array.from(container.querySelectorAll('.barang-row')).find(row => {
                            const selectedOption = row.querySelector('.barang-select')
                                .selectedOptions[0];
                            return selectedOption && selectedOption.getAttribute('data-kode') ===
                                kode;
                        });
